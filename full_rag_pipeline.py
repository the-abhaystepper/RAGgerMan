...
def rag_generate(chat_agent, retriever, user_query: str, chat_history: list = None):
    ...
    # 1. History-aware query reformulation if conversation exists
    standalone_query = create_history_aware_query(chat_agent, chat_history, user_query)

    # 2. Retrieve documents using standalone query with MMR search
    retrieved_docs = retriever.invoke(standalone_query)

    # 3. Format combined input with retrieved context
...
"""
    ...
    # 4. Invoke the chat model and return answer and source documents
...
def main():
    ...
    doc_path = "docs/pdfs/attention-is-all-you-need.pdf"

    print("1. Loading documents...")
    documents = load_documents(doc_path)
    
    print("2. Splitting documents into chunks...")
    chunks = split_documents(documents=documents, chunk_size=500, overlap=50)
    
    print("3. Embedding and storing chunks into ChromaDB...")
    db = embed_and_store(chunks=chunks, store_directory=store_dir)
    
    print("4. Loading Chat Model...")
    ...
    query = "What is the formula for multi-head attention?"
    print(f"\nAsking query: {query}")
    result = rag_generate(chat_agent, retriever, query)
